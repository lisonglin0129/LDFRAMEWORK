...
class Smarty extends Smarty_Internal_TemplateBase
{
    ...
    public function __construct()
    {
        ...
        if ($this->tp_path !== './cache/') {
            unset(self::$_muted_directories['./cache/']);
            $this->setCacheDir($this->tp_path);
        }
     ...
     * Set cache directory
     *
     * @param  string $tp_path directory to store cached templates in
     *
     * @return Smarty current Smarty instance for chaining
     */
    public function setCacheDir($tp_path)
    {
        $this->tp_path = rtrim($tp_path, '/\\') . DS;
        if (!isset(Smarty::$_muted_directories[$this->tp_path])) {
            Smarty::$_muted_directories[$this->tp_path] = null;
        }
        return $this;
    }
